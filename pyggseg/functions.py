...
def ggseg_plot_glasser_bluered(values, metric, output_file, maxscale = None, title = None):
	values = np.array(values)
	assert len(values) == 358, "Error: the data length must be 358"
	assert values.ndim == 1, "Error: the data must be either 1d array or list"
	if maxscale is None:
		maxscale = np.ceil(np.abs(values).max() * 1000.) / 1000.
		print("Setting maximum scale to: %1.3f" % maxscale)
	convert_glasser(atlasdata = values, dataname = metric)
	cmd = ("""
	ggplot(ratlasdata) +
	geom_brain(atlas = glasser, 
...
def convert_rywlbb_gradient():
	cmd = ("""scale_fill_gradientn(colors = c("#00008C", "#2234A8", "#4467C4", "#659BDF", "#87CEFB", "white", "#ffec19", "#ffc100", "#ff9800", "#ff5607", "#f6412d"))""")
	grad = ro.r(cmd)
	crange = np.linspace(0, 1, 256)
	cmap = np.zeros((len(crange), 4))
	for v, value in enumerate(crange):
		cmap[v] = np.concatenate((np.array(ImageColor.getrgb(np.array(grad['palette'](float(value)))[0])),[255]))
	cmap = cmap/255
	return(cmap)
